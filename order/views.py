...
@login_required(login_url='/login')  # checks login
def addtoshopcart(request, id):
    ...
    if request.method == 'POST':  # if there is a post
        ...
        if control == 1:  # Update  shopcart
            ...
            data = ShopCart()  # connect with model
            data.user_id = current_user.id
            data.product_id = id
            data.quantity = 1
        ...
        if form.is_valid():
            ...
            for rs in shopcart:
                ...
                if rs.product.variant == 'None':
                    ...
                    variant = Variants.objects.get(id=rs.product_id) # it used to be (id=rs.product_id)
