...
class ShopCart(models.Model):
    ...
    @property
    def price(self):
        if self.product_id is not None: # just got this from stack overflow
    ...
    @property
    def amount(self):
        if self.product_id is None: # just got this from stack overflow
            return '0'
        return self.quantity * self.product.price

    @property
    def varamount(self):    # just got this from stack overflow
        if self.variant_id is not None:
            return self.quantity * self.variant.price'''
